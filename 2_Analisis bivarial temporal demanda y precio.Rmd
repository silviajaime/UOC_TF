...
ggplot(s_hora, aes(x=hour, y=PM)) + 
    ...
    geom_text(aes(label = round(PM, 1)), color="black", size=4.5)+
    ggtitle("Mean y standard deviation bars hora y precio medio")
...
```{r}
#Realizamos un grafico de medias por hora con intervalos de confianza mediante la funcion plotmeans
plotmeans(PM~electricity_prices$Hour, digits=1, ccol="red", mean.labels=T,bars=T, xlab= "Hora",n.label=F, ylab= "Mean precio medio", main="Mean precio medio por hora") 
```
Aparece un error debido a que los intervalos de confianza son muy pequenos para el grafico.
...
ggplot(s_month, aes(x=Month, y=PM)) + 
    ...
    geom_text(aes(label = round(PM, 1)), color="black", size=3.5) + 
    ggtitle("Mean y SD bars precio y mes")
